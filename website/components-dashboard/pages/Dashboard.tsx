...
export default function Dashboard() {
  ...
  const { can } = useAuth()
  const canViewVendors = can('manage_vendors')
  const canViewErrors = can('view_error_logs')
  ...
  const { data: errorLogs, isLoading: errLoading } = useQuery({
    ...
    enabled: canViewErrors,
  })

  const pendingInvoices = invoices?.filter((i) => i.status === 'Pending').length || 0
  const vendorsCount = canViewVendors ? vendors?.length || 0 : 0
  const errorLogsCount = canViewErrors ? errorLogs?.length || 0 : 0
